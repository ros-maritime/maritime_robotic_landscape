#!/usr/bin/env python3
"""
This script updates the dates in markdown tables within maritime landscape files.

It performs the following steps:

1. Scans markdown files for table rows.
...
# ───────────── Configuration ───────────────────────────────────────
MARKDOWN_FILES = [
    Path("docs/autonomy_stacks.md"),
    Path("docs/simulation.md"),
    Path("docs/software.md"),
]
...
def process_file(markdown_file: Path) -> None:
    """Process a single markdown file and update GitHub-linked table rows."""
    if not markdown_file.exists():
        print(f"Warning: Markdown file '{markdown_file}' not found. Skipping.")
        return
...
def main() -> None:
    """Process all configured markdown files and update GitHub-linked table rows."""
    print("Maritime Robotics Landscape - Repository Date Updater")
    print("=" * 60)
